    ...
    if "NIFTI" in scanLabels and "BIDS" in scanLabels:
        ...
        if ("PA" in splitdesc or "AP" in splitdesc) and "topup" in splitdesc:
            ...
            for desc in split_desc:
                if desc.startswith("Fieldmap") and is_digit(desc[-1]):
            ...
            for label in ["BIDS","NIFTI"]:
                ...
                if key is params.keys()
                    assert "fmap_AP_file" in params.keys(), "Error missing fmap_AP_file before changing dico_file"
                    assert "fmap_PA_file" in params.keys(), "Error missing fmap_PA_file before changing dico_file"

                    assert "json_AP_file" in params.keys(), "Error missing json_AP_file before changing dico_file"
                    assert "json_PA_file" in params.keys(), "Error missing json_PA_file before changing dico_file"

                    print("Storing cur dict_files {} for {}".format(" ".join(dico_files.keys(), index_run)
                    dict_dico_files[index_run] = dico_files
        ...
        if splitdesc[-1] != "SBRef" and splitdesc[0].startswith("task-"):
            ...
            if 'Name' in pathDict.keys():
                ...
                print("IntendedFor: {}".format(dico_files['IntendedFor']))
...
fout_file=os.path.join(topupdir,subject+"_fieldmap")
...
topup_json_file=os.path.join(topupdir,subject+"_fieldmap.json")
...
mag_file = os.path.join(topupdir,subject+"_magnitude")
...
try:
    ...
    r = sess.delete(
        host+"/data/experiments/%s/reconstructions/TOPUP_JSON_%s" %(session, subject),
        params=queryArgs)
...
r = sess.put(host+ "/data/experiments/%s/reconstructions/TOPUP_JSON_%s?\
    xnat:reconstructedImageData"%(session, subject), params=queryArgs)
r.raise_for_status()
...
r = sess.put(host+"/data/experiments/%s/reconstructions/TOPUP_JSON_%s/files"%(session, subject), params=queryArgs)
r.raise_for_status()
...
try:
    ...
    r = sess.delete(host+"/data/experiments/%s/reconstructions/TOPUP_FMAP_%s" % (
        session, subject), params=queryArgs)
    r.raise_for_status()
